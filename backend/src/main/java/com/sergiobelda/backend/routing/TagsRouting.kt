...
fun Route.tagsRouting(tagService: ITagService) {
    route("/v1/tags") {
        ...
        get("/{id}") {
            ...
            val tagId = call.parameters["id"] ?: throw IllegalStateException("Must provide id")
            call.respond(tagService.getTag(tagId))
        }
        post {
            val tag = call.receive<NewTag>()
            call.respond(tagService.insertTag(tag))
        }
        delete("/{id}") {
            // TODO respond error if id not present
            val tagId = call.parameters["id"] ?: throw IllegalStateException("Must provide id")
